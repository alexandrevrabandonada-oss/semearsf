    ...
    return (
        <section className="space-y-6">
            <div className="rounded-2xl border border-border-subtle bg-white p-6 shadow-sm md:p-8">
                ...
                <p className="mt-2 text-base leading-relaxed text-text-secondary">{meta.description}</p>
            </div>
                                ...
                                >
                                    <div className="flex flex-wrap items-start justify-between gap-2">
                                        <div className="flex items-center gap-2">
                                            <span className="shrink-0 rounded-full bg-brand-primary/10 px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-brand-primary">
                                                {KIND_LABELS[item.kind]}
                                            </span>
                                            {item.source_type && (
                                                <span className="shrink-0 rounded-full bg-accent-green/10 px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-accent-green">
                                    ...
                                    {item.authors && (
                                        <p className="text-xs font-semibold text-text-secondary italic">Por: {item.authors}</p>
                                    )}
                                    {item.excerpt && (
                                        <p className="line-clamp-2 text-sm leading-relaxed text-text-secondary">{item.excerpt}</p>
                                    )}
                                    {item.source_name && (
                                        <p className="text-xs text-text-secondary">Fonte: {item.source_name}</p>
                                    )}
                                    {item.tags.length > 0 && (
                                        <div className="mt-1 flex flex-wrap gap-1">
                                            {item.tags.map((tag) => (
                                                <span
                                                    className="rounded-full border border-border-subtle px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-brand-primary"
                                                    key={tag}
                                                >
                                                    {tag}
                                                </span>
